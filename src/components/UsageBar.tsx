      ...
      <div className="flex justify-between text-[11px] font-medium text-muted-foreground select-none">
        ...
        {resetsAt ? (
          <Tooltip>
            ...
            <TooltipContent side="top" className="text-[10px] bg-popover border border-border text-foreground py-1 px-2 rounded">
              Resets on {exactResetLabel}
            </TooltipContent>
          </Tooltip>
        ) : (
...
export function UsageBar({ usage, loading }: UsageBarProps) {
  if (loading && !usage) {
    return (
      <div className="space-y-2">
        ...
        <div className="h-1.5 bg-muted rounded-full overflow-hidden animate-pulse">
          <div className="h-full w-2/3 bg-muted-foreground/30"></div>
        </div>
      </div>
    );
  }
  ...
  if (!usage) {
    return (
      <div className="text-[11px] text-muted-foreground italic py-1 animate-pulse">
        Fetching usage...
      </div>
    );
  }
  ...
  if (usage.error) {
    return (
      <div className="text-[11px] text-destructive italic py-1">
        {usage.error}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2.5">
      ...
      {usage.credits_balance && (
        <div className="text-[11px] text-muted-foreground font-medium select-none">
